...
function Downline() {
  ...
  try {
    ...
    const [adminsResponse, clientsResponse] = await Promise.all([
      ...
      axios.get<ApiResponse>(`${config.baseURL}/client/downline/${userData.id}`, {
        headers: { Authorization: `Bearer ${token}` }
      }).catch(err => {
        console.error('Client downline error:', err);
        return { data: { success: false, data: [] } };
      })
    ]);
    ...
    const clientUsers = clientsResponse.data.success
      ? clientsResponse.data.data.map(client => processUser(client, 'client'))
      : [];

    const combinedUsers = [...adminUsers, ...clientUsers];
